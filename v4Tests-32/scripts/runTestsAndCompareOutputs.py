...
def mysystem(language, cmd, bCanFail):
    ...
    if ret != 0 and not bCanFail:
        PrintFailed(cmd)
        mysystem(language, "cat tmp.err"+"_"+language, True)
        sys.exit(1)
    return ret
...
def RunTestCase_exec_asn1scc(language, targetDir, asn1, acn, behavior, expErrMsg):    
    global nTests
...
def RunTestCase(language, targetDir, asn1, acn, behavior, expErrMsg):
    global nTests
    
    execute_test_case = RunTestCase_exec_asn1scc(language, targetDir, asn1, acn, behavior, expErrMsg)
    
    if not execute_test_case:
        return
    ...
    if language == "c":
        ...
        res = mysystem(language, "make coverage >covlog.txt 2>&1", True)
        if res != 0 and behavior != 2:
            PrintFailed("run time failure")
            PrintFailed("covlog.txt is ...")
            mysystem(language, "cat covlog.txt",False)
            sys.exit(1)
...
def RunTestCaseBothLang(asn1, acn, behavior, expErrMsg):
    RunTestCase("Ada", rootDir + os.sep + "tmp_Ada", asn1, acn, behavior, expErrMsg);
    RunTestCase("c", rootDir + os.sep + "tmp_c", asn1, acn, behavior, expErrMsg);
    mysystem("c","find . -name *dat -exec sh -c 'hexdump {} > {}.txt' \;",False);
    subprocess.call("find . -name *dat.txt  > test_cases_dumplist.txt", shell=True)

    lines = list(open("test_cases_dumplist.txt", 'r').read().splitlines());
    items = list(map(lambda x: {'abs_path' : x, 'fname' : ntpath.basename(x)}, lines))
    grouped = collections.defaultdict(list)
    for item in items:
        grouped[item['fname']].append(item)    
        
    for fname, group in grouped.items():
        if len(group) == 2:
            print ("----------------------------- comparing " + fname+ " case ----------")
            cmd = "diff " + group[0]['abs_path'] + " " + group[1]['abs_path']
            #subprocess.call(cmd, shell=True)
            mysystem("c", cmd, False)

    
def DoWork_ACN(language, asn1file, targetDir):
    ...
    for line in lines:
        if line.find("--TCLS") == 0:
            for l in ["c", "Ada"]:
                targetDir = rootDir + os.sep + "tmp_" + l
                ...
                CreateACNFile(targetDir, tmp_line)
                #RunTestCase(language, targetDir, os.sep.join(asn1file.split(os.sep)[-2:]), tmp_line, 0, "")
            RunTestCaseBothLang(os.sep.join(asn1file.split(os.sep)[-2:]), tmp_line, 0, "")
            ...
            CreateACNFile(targetDir, tmp_line)
            RunTestCase(language, targetDir, os.sep.join(asn1file.split(os.sep)[-2:]), tmp_line, 1, tmp_err)
            ...
            tmp_line = tmp_line.split("$$$")[0].strip()
            CreateACNFile(targetDir, tmp_line)
            RunTestCase(language, targetDir, os.sep.join(asn1file.split(os.sep)[-2:]), tmp_line, 2, tmp_err)
        # # for file (TCF)
...
def submain(lang, encoding, testCaseSet, cntTest):
    ...
    if os.path.isfile(testCaseSet):
        globals()[funcName](language, os.path.abspath(testCaseSet), targetDir)
    else:
        ...
        for curDir in sorted(os.listdir(testCaseSet)):
            ...
            for asn1file in asn1files:
                ...
                if cntTest and testCaseStart != "" and relAsn1Path < testCaseStart:
                    ...
                    globals()[funcName](
                        language, os.path.abspath(
                            testCaseSet + os.sep + curDir + os.sep + asn1file), targetDir)

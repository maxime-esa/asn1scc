...
package adaasn1rtl with
   ...
   function String_Equal (Left, Right : String) return Boolean with
      Pre => Left'Last < Natural'Last
      and then Left'Last >= Left'First
      and then Right'Last < Natural'Last
      and then Right'Last >= Right'First;

   function Asn1Real_Equal
   ...
   function OctetString_equal
     (len1 : Integer;
      len2 : Integer;
      arr1 : OctetBuffer;
      arr2 : OctetBuffer) return Boolean is
     (len1 = len2
      and then
   ...
   function BitString_equal
     (len1 : Integer;
      len2 : Integer;
      arr1 : BitArray;
      arr2 : BitArray) return Boolean is
     (len1 = len2
      and then
        arr1 (arr1'First .. arr1'First + (len1 - 1)) =
        arr2 (arr2'First .. arr2'First + (len1 - 1))) with

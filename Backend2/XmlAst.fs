    ...
    let PrintTypeAux (t:Asn1Type) =
        ...
        | Choice(children)      -> 
            let emitChild (c:ChildInfo) =
                xml.ChoiceChild c.Name.Value (ToC c.Name.Value) (BigInteger c.Name.Location.srcLine) (BigInteger c.Name.Location.charPos) (PrintType c.Type modName r) (c.CName_Present C)
            xml.ChoiceType (children |> Seq.map emitChild)
        | Sequence(children)    -> 
            let emitChild (c:ChildInfo) =
                match c.Optionality with
                | Some(Default(vl)) -> xml.SequenceChild c.Name.Value (ToC c.Name.Value) true (PrintAsn1.PrintAsn1Value vl) (BigInteger c.Name.Location.srcLine) (BigInteger c.Name.Location.charPos) (PrintType c.Type modName r)
                | _                 -> xml.SequenceChild c.Name.Value (ToC c.Name.Value) c.Optionality.IsSome null (BigInteger c.Name.Location.srcLine) (BigInteger c.Name.Location.charPos) (PrintType c.Type modName r)
            xml.SequenceType (children |> Seq.map emitChild)
        | Enumerated(items)     -> 
            let emitItem (idx: int) (it : Ast.NamedItem) =
                match it._value with
                | Some(vl)  -> xml.EnumItem it.Name.Value (ToC it.Name.Value) (Ast.GetValueAsInt vl r) (BigInteger it.Name.Location.srcLine) (BigInteger it.Name.Location.charPos) (it.CEnumName r C)
                | None      -> xml.EnumItem it.Name.Value (ToC it.Name.Value) (BigInteger idx) (BigInteger it.Name.Location.srcLine) (BigInteger it.Name.Location.charPos) (it.CEnumName r C)
...
let DoWork (r:AstRoot) =
    ...
    let PrintModule (m:Asn1Module) =
        ...
        xml.ModuleXml m.Name.Value (ToC m.Name.Value) (m.Imports |> Seq.map PrintImpModule) m.ExportedTypes m.ExportedVars (m.TypeAssignments |> Seq.map (fun t -> PrintTas t m.Name.Value)) (m.ValueAssignments |> Seq.map (fun t -> PrintVas t m.Name.Value)) (AllExported m)

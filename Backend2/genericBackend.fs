...
let DoWork (r:AstRoot) (stgFileName:string) (outFileName:string) =
    let PrintVas (vas: Ast.ValueAssignment) modName =
        gen.VasXml vas.Name.Value (BigInteger vas.Name.Location.srcLine) (BigInteger vas.Name.Location.charPos) (PrintType vas.Type modName r stgFileName) stgFileName
    ...
    let PrintModule (m:Asn1Module) =
        ...
        gen.ModuleXml m.Name.Value (m.Imports |> Seq.map PrintImpModule) m.ExportedTypes m.ExportedVars (m.TypeAssignments |> Seq.map (fun t -> PrintTas t m.Name.Value)) (m.ValueAssignments |> Seq.map (fun t -> PrintVas t m.Name.Value)) stgFileName

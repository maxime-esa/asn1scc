...
def CalculateMantissaAndExponent(d: Int, exponent: Ref[Int], mantissa: Ref[ULong]): Unit = { // STAINLESS: d: Double
...
def GetDoubleByMantissaAndExp(mantissa: ULong, exponentVal: Int): Int = { // STAINLESS return Double
  /*
...
def BitStream_EncodeReal(pBitStrm: BitStream, vVal: Int): Unit = { // STAINLESS vVal: Double
  /*
    ...
    val exponent = Ref[ULong](0)
    val mantissa = Ref[ULong](0)

    CalculateMantissaAndExponent(v, exponent, mantissa)
    val nExpLen: Int = GetLengthInBytesOfSInt(exponent.x.)
...
def BitStream_DecodeReal(pBitStrm: BitStream, v: Ref[Int]): Boolean = { // STAINLESS: v: Ref[Double]
...
def DecodeRealAsBinaryEncoding(pBitStrm: BitStream, lengthVal: Int, header: UByte, v: Ref[Int]): Boolean = { // STAINLESS: v: Ref[Double]

...
let allocatedValuesToAllEnumItems (namedItems:NamedItem list) ast = 
    ...
    let rec allocateItems (unallocated:NamedItem list) (allocated:NamedItem list) potentialValue: NamedItem list =
        ...
        |x::xs  -> 
            let rec getValue (allocated:NamedItem list) (vl:BigInteger) =
                match allocated |> Seq.exists(fun ni -> match ni._value with Some value -> vl = (Ast.GetValueAsInt value ast) | None -> false) with
                | false -> vl
...
let numericStringDefaultConstraints () =
    ...
    AlphabetContraint(UnionConstraint(RangeContraint(zero,nine, true, true),SingleValueContraint(space), false))

let CloneType (old:Asn1Type) m (key:list<string>) (cons:Constructors<State>) (state:State) =
    match old.Kind with
    |NumericString  -> 
        let newConstraint = numericStringDefaultConstraints()
        {old with Kind = IA5String; Constraints = newConstraint::old.Constraints}, state
    | Enumerated(namedItems) ->
        let newItems = allocatedValuesToAllEnumItems namedItems state

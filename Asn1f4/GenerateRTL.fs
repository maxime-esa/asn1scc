    ...
    | ProgrammingLanguage.Ada ->
        ...
        | _     ->
            let adaasn1rtl_encoding_adb = 
                ...
                | true  -> rm.GetString("adaasn1rtl_encoding_adb",null).Replace("--  with user_code;","with user_code;").Replace("--  bs.Current_Bit_Pos := 0;","bs.Current_Bit_Pos := 0;").Replace("--  user_code.push_data(bs, bs.pushDataPrm);","user_code.push_data (bs, bs.pushDataPrm);").Replace("--  user_code.fetch_data(bs, bs.fetchDataPrm);","user_code.fetch_data (bs, bs.fetchDataPrm);")

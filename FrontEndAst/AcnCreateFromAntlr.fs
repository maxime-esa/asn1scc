...
and private ChildSpec = {
    name            : StringLoc option
    childEncodingSpec : AcnTypeEncodingSpec
    asn1Type        : Asn1AcnAst.AcnAsn1Type option    // if present then it indicates an ACN inserted type
...
let private CreateAcnAsn1Type (t:ITree) =
    match t.Type with
    | acnParser.INTEGER         -> Asn1AcnAst.AcnAsn1Type.AcnInteger t.Location
    | acnParser.BOOLEAN         -> Asn1AcnAst.AcnAsn1Type.AcnBoolean t.Location
    | acnParser.NULL            -> Asn1AcnAst.AcnAsn1Type.AcnNullType t.Location
    | acnParser.REFERENCED_TYPE -> 
        ...
        Asn1AcnAst.AcnAsn1Type.AcnRefType(mdName, tsName)
...
let private GetParams (files:AntlrParserResult list) modName tasName  =
...
let rec  private createTypeEncodingSpec (allAcnFiles: AntlrParserResult list) (acnConstants : Map<string, BigInteger>) (thisAcnFile: AntlrParserResult)  (encSpecITree:ITree) : AcnTypeEncodingSpec =
    let acnProperties = 
        ...
        | Some childrenList ->
            let createChild (t:ITree) =
                ...
                let asn1Type  =
                    ...
                    | acnParser.CHILD_NEW   -> Some (CreateAcnAsn1Type (t.GetChild 1) ) 
    ...
    {AcnTypeEncodingSpec.acnProperties = acnProperties; children = children}

let private CreateTypeAssignment (allAcnFiles: AntlrParserResult list) (acnConstants : Map<string, BigInteger>) (thisAcnFile: AntlrParserResult)  (tasTree:ITree) : AcnTypeAssignment =
    ...
    let prms = 
        ...
        | Some(paramList) -> 
            let CreateParam (x:ITree) =
                ...
                | Some(_)   -> {Asn1AcnAst.AcnParameter.name = prmName; Asn1AcnAst.AcnParameter.asn1Type=CreateAcnAsn1Type (x.GetChild(0)) ; Asn1AcnAst.AcnParameter.loc = loc}
...
let private CreateModule (allAcnFiles: AntlrParserResult list) (acnConstants : Map<string, BigInteger>) (thisAcnFile: AntlrParserResult)   (modTree : ITree) : AcnModule =
...
let private LoadAcnFile (allAcnFiles: AntlrParserResult list) (acnConstants : Map<string, BigInteger>) (thisAcnFile: AntlrParserResult)   : AcnFile = 
...
let private CreateAcnAst (allAcnFiles: AntlrParserResult list) : AcnAst =  

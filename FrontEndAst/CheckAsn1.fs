...
let c_keywords =  CommonTypes.c_keywords
let scala_keywords =  CommonTypes.scala_keywords
let ada_keywords =  CommonTypes.ada_keywords
...
///checks whether the input values is int or not
...
let rec IsValueAllowed (c:Asn1Constraint) (v:Asn1Value) (isOfEnumType:bool) (bitOrOctSrt:TYPE_BIT_OR_OCT_STR option) (ast:AstRoot) =
    ...
    match c with
    | SingleValueConstraint(_, v1)          -> AreAsn1ValuesEqual v1 v isOfEnumType ast
    | RangeConstraint(_, v1, v2, minInclusive, maxInclusive)            ->
        match minInclusive, maxInclusive with
    ...
    | RangeConstraint_val_MAX(_, v1, minInclusive)        ->
        match minInclusive with
        | true      -> CompareAsn1Value v1 v ast <=0
        | false     -> CompareAsn1Value v1 v ast <0
    | RangeConstraint_MIN_val(_, v2, maxInclusive)        ->
        match maxInclusive with
        | true  -> CompareAsn1Value v v2 ast <= 0
        | false -> CompareAsn1Value v v2 ast < 0
    | RangeConstraint_MIN_MAX            -> true
    | SizeConstraint(_, sc)                 ->
        let rec IsSizeConstraintOK (v:Asn1Value) (sc:Asn1Constraint) =
            match v.Kind with
            | StringValue(s, l)                ->
            ...
            | RefValue(modName,vasName)                  -> IsSizeConstraintOK (GetBaseValue modName vasName ast) sc
            | _                             -> raise (BugErrorException(""))
        IsSizeConstraintOK v sc
    | AlphabetConstraint(_, ac)             ->
        let rec IsAlphabetConstraintOK (v:Asn1Value) (ac:Asn1Constraint) =
            match v.Kind with
            | StringValue(s, l)    ->
                let sValue = StringValue2String s
                match ac with
                | SingleValueConstraint (_, setVal)   ->
        ...
        let rec IsWithComponentsConstraintOK (v:Asn1Value) =
            ...
            | SeqValue(children)    ->
                let IsNamedConstraintOK (nc:NamedConstraint) =
                    ...
                    match nc.Mark, ch, nc.Constraint with
                    | NoMark, None, _                       -> true
            ...
            | ChValue(aName, inVal)    ->
                ...
                | None      -> true
                | Some(rnc) ->
                    match rnc.Mark, rnc.Constraint with
                    | MarkAbsent, _     -> false
...
let rec isConstraintValid (t:Asn1Type) (c:Asn1Constraint) ast =
    let rec CanHaveRangeConstraint (t:Asn1Type) =
        match t.Kind with
        ...
        | ReferenceType(_)                                              -> CanHaveRangeConstraint (GetActualType t ast)

    let rec CanHaveSizeConstraint (t:Asn1Type) =
        match t.Kind with
        ...
        | ReferenceType(_)                                                                  -> CanHaveSizeConstraint (GetActualType t ast)
    let rec CanHaveFromConstraint (t:Asn1Type) =
        match t.Kind with
        ...
        | ReferenceType(_)                                                                            -> CanHaveFromConstraint (GetActualType t ast)
    ...
    match c with
    | SingleValueConstraint(_, v1)          ->  CheckValueType t v1 ast
    | RangeConstraint(_, v1,v2,_,_)             ->
        if not(CanHaveRangeConstraint t) then
            raise(SemanticError(t.Location, "Type does not support range constraints"))
        CheckValueType t v1 ast
        CheckValueType t v2 ast
    | RangeConstraint_val_MAX(_, v1,_)
    | RangeConstraint_MIN_val(_, v1,_)       ->
        if not(CanHaveRangeConstraint t) then
            raise(SemanticError(t.Location, "Type does not support range constraints"))
        CheckValueType t v1 ast
    | RangeConstraint_MIN_MAX           ->
        if not(CanHaveRangeConstraint t) then
            raise(SemanticError(t.Location, "Type does not support range constraints"))
    | SizeConstraint(_, c1)                 ->
        if not(CanHaveSizeConstraint t) then
    ...
    | AlphabetConstraint(_, c1)             ->
        if not(CanHaveFromConstraint t) then

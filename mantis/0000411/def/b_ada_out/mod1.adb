...
-- } (((red) | (cyan)) | (magenta))
    ...
    ret := adaasn1rtl.ASN1_RESULT'(Success => (((val = red) OR (val = cyan)) OR (val = magenta)), ErrorCode => ERR_OTHERCOLORS);
    RETURN ret;
...
BEGIN
    RETURN CH2'(kind => left_PRESENT, left => itm);
END CH2_left_set;
...
begin
    ...
    ((if CH2_kind(val1) = left_PRESENT then

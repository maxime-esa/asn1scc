...
-- } (((red) | (green)) | (blue))
    ...
    ret := adaasn1rtl.ASN1_RESULT'(Success => (((val = RGBCOLORS_red) OR (val = RGBCOLORS_green)) OR (val = RGBCOLORS_blue)), ErrorCode => ERR_RGBCOLORS);
    RETURN ret;
...
BEGIN
    ...
    IF result.success THEN
        ...
        case val is 
            ...
            when RGBCOLORS_green    => intVal := 1;
            when RGBCOLORS_blue    => intVal := 2;
        end case;
...
BEGIN
    ...
    IF result.Success THEN
        case RGBCOLORS_index_range(intVal) is
            ...
            when 1 => val := RGBCOLORS_green;
            when 2 => val := RGBCOLORS_blue;
        end case;
    END IF;
...
-- } (((red) | (cyan)) | (magenta))
    ...
    ret := adaasn1rtl.ASN1_RESULT'(Success => (((val = OTHERCOLORS_red) OR (val = OTHERCOLORS_cyan)) OR (val = OTHERCOLORS_magenta)), ErrorCode => ERR_OTHERCOLORS);
    RETURN ret;
...
BEGIN
    ...
    IF result.success THEN
        ...
        case val is 
            ...
            when OTHERCOLORS_cyan    => intVal := 1;
            when OTHERCOLORS_magenta    => intVal := 2;
        end case;
        adaasn1rtl.UPER_Enc_ConstraintWholeNumber(S, K, intVal, 0, 2);
    END IF;
...
BEGIN
    ...
    IF result.Success THEN
        case OTHERCOLORS_index_range(intVal) is
            ...
            when 1 => val := OTHERCOLORS_cyan;
            when 2 => val := OTHERCOLORS_magenta;
        end case;
    END IF;
...
BEGIN
    RETURN CH2'(kind => CH2_center_PRESENT, center => itm);
END CH2_center_set;
...
(if CH2_kind(val1) = CH2_center_PRESENT then
...
BEGIN
    RETURN CH1'(kind => CH1_right_PRESENT, right => itm);
END CH1_right_set;
...
(if CH1_kind(val1) = CH1_right_PRESENT then

    ...
    seq {
            for (i1,(a,b))  in (toIdxList pairs) do
                ...
                for (i2, c) in (toIdxList (None::cons)) do
                    let asn1Type =  {ParameterizedAsn1Ast.Asn1Type.Kind = ParameterizedAsn1Ast.Integer; ParameterizedAsn1Ast.Constraints = Option.toList c ; ParameterizedAsn1Ast.Location=emptyLocation; parameterizedTypeInstance = false; acnInfo = None}    

...
(*
let rec printValue (r:DAst.AstRoot)  (lm:LanguageMacros) (lm:LanguageMacros) (curProgamUnitName:string)  (t:Asn1Type) (parentValue:Asn1ValueKind option) (gv:Asn1ValueKind) =
    match l with
    | C ->
        match gv with
        | IntegerValue      v -> lm.vars.PrintIntValue v
        | RealValue         v -> lm.vars.PrintRealValue v
        | StringValue       v -> lm.vars.PrintStringValue v []
        | BooleanValue      v -> lm.vars.PrintBooleanValue v
        | BitStringValue    v -> 
            let bytes = bitStringValueToByteArray (StringLoc.ByValue v)
            match t.ActualType.Kind with
            | OctetString os    -> 
                ...
                lm.vars.PrintOctetStringValue td (os.baseInfo.minSize.uper = os.baseInfo.maxSize.uper) bytes (BigInteger v.Length)
            | BitString   bs    -> 
        ...
        | EnumValue         v -> 
            ...
            | Enumerated enm    -> 
                let itm = enm.baseInfo.items |> Seq.find(fun x -> x.Name.Value = v)
                lm.vars.PrintEnumValue (itm.getBackendName (Some t.typeDefintionOrReference) l)
            | _         -> raise(BugErrorException "unexpected type")
        | NullValue         v -> lm.vars.PrintNullValue ()
        | SeqOfValue        v -> 
            match t.ActualType.Kind with
        ...
        | SeqValue          v -> 
            ...
            | Sequence s -> 
                let optChildren = 
                    ...
                    List.map(fun x ->
                        match v |> Seq.tryFind(fun chv -> chv.name = x.Name.Value) with
                        | Some _    -> lm.vars.PrintSequenceValue_child_exists (x.getBackendName l) "1"
                        | None      -> lm.vars.PrintSequenceValue_child_exists (x.getBackendName l) "0")
                let arrChildren = 
                    s.children |>
                    List.choose(fun ch -> match ch with Asn1Child a -> Some a | AcnChild _ -> None) |>
                    List.choose(fun cht -> 
                        match v |> Seq.tryFind(fun chv -> chv.name = cht.Name.Value) with
                        | Some v    -> Some (lm.vars.PrintSequenceValueChild (cht.getBackendName l) (printValue r l lm curProgamUnitName cht.Type (Some gv) v.Value.kind))
                        | None      -> 
                            match cht.Optionality with
                            | Some(Asn1AcnAst.Optional opt)    -> 
                                match opt.defaultValue with
                                | Some v    -> Some (lm.vars.PrintSequenceValueChild (cht.getBackendName l) (printValue r l lm curProgamUnitName cht.Type (Some gv) (mapValue v).kind ))                    
                                | None      -> None
                            | _             -> None)
        ...
        | ChValue           v -> 
            ...
            | Choice s -> 
                ...
                List.map(fun x -> 
                    ...
                    lm.vars.PrintChoiceValue typeDefName (x.getBackendName l) childValue   sChildNamePresent true) |>
                List.head
            | _         -> raise(BugErrorException "unexpected type")
        | ObjOrRelObjIdValue v  ->
            match t.ActualType.Kind with
    ...
    | Ada ->
        ...
        | EnumValue         v -> 
            ...
            | Enumerated enm    -> 
                ...
                | true  -> lm.vars.PrintEnumValue (itm.getBackendName (Some t.typeDefintionOrReference) l)
                | false -> lm.vars.PrintEnumValue  ((ToC typeModName) + "." + (ToC itm.ada_name))
            | _         -> raise(BugErrorException "unexpected type")
        | NullValue         v -> lm.vars.PrintNullValue ()
        | ObjOrRelObjIdValue v  ->
            match t.ActualType.Kind with
        ...
        | SeqValue          v -> 
            ...
            | Sequence s -> 
                ...
                let arrChildren = 
                    ...
                    List.map(fun x -> 
                        match v |> Seq.tryFind(fun chv -> chv.name = x.Name.Value) with
                        | Some v    -> lm.vars.PrintSequenceValueChild (x.getBackendName l) (printValue r l lm curProgamUnitName x.Type (Some gv) v.Value.kind)
                        | None      -> 
                            let chV = 
                                match x.Optionality with
                                | Some(Asn1AcnAst.Optional opt)    -> 
                            ...
                            lm.vars.PrintSequenceValueChild (x.getBackendName l) chV )
                //let allChildren = match Seq.isEmpty optChildren with
                //                  | true     -> arrChildren
                //                  | false    -> arrChildren @ [lm.vars.PrintSequenceValue_Exists td optChildren]
                //lm.vars.PrintSequenceValue typeDefName allChildren
                lm.vars.PrintSequenceValue td typeDefName arrChildren optChildren
            | _         -> raise(BugErrorException "unexpected type")
        | ChValue           v -> 
            match t.ActualType.Kind with
            | Choice s -> 
                ...
                s.children |>
                List.filter(fun x -> x.Name.Value = v.name)  |>
                List.map(fun x -> lm.vars.PrintChoiceValue typeDefName (x.getBackendName l) (printValue r l  lm curProgamUnitName x.chType (Some gv) v.Value.kind) (x.presentWhenName (Some t.typeDefintionOrReference) l) true ) |>
                List.head
            | _         -> raise(BugErrorException "unexpected type")
